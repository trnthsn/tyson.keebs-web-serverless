...
export const SaveLoadView = ({
  ...
  return (
    <div className="h-full overflow-y-auto px-8 py-6">
      ...
      <button
        onClick={saveLayout}
        className="w-full py-3 mb-4 text-[1.6rem] uppercase tracking-wide rounded-[0.4rem] border border-[#E8C4B8] bg-[#E8C4B8] text-[#363434] hover:opacity-90 transition-opacity"
      >
        {t('tysonkeeb.saveLayout')}
      </button>
      <button
        onClick={() => fileInputRef.current?.click()}
        className="w-full py-3 text-[1.6rem] uppercase tracking-wide rounded-[0.4rem] border border-[#E8C4B8] text-[#222] dark:text-[#d9d9d9] hover:bg-[#ebe4e4] dark:hover:bg-[#333] transition-colors"
      >
